...
function getAutoStatus(flight: Flight): string | null {
  ...
  // v4.5 FIX: raniji uslov je odustajao čim je StatusEN bio BILO ŠTA osim "-",
  // a API (ngrok i sekundarni izvor) skoro uvijek vraća doslovno "On Time"
  // za let koji još nije aktivan. Rezultat: auto-status se nikad nije računao,
  // pa je tabla stalno prikazivala samo "On Time" umjesto Check-In/Go to
  // Gate/Final Call/Close. Sad se generički status i dalje tretira kao
  // "još nema pravog statusa" i računa se auto-status.
          ...
          <div className="flex items-center justify-center" style={{ width: '640px' }}>
            {pill.hasStatusText ? (
              <div className={`${pillCls}`}>
                ...
                <span className="whitespace-nowrap">{pill.displayText}</span>
              </div>
            ) : (
...
useEffect(() => {
  ...
  const rawDepartures = getUniqueDeparturesWithDeparted(
    filterRecentDepartures(liveFlightData.departures || [])
  );
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
}, [liveFlightData, filterRecentDepartures]);
...
useEffect(() => {
  ...
  if (!liveFlightData) return;
  // eslint-disable-next-line react-hooks/set-state-in-effect
  setFlights(prev => applyAssignmentsOnly(prev, assignments));
  // eslint-disable-next-line react-hooks/exhaustive-deps
}, [assignments]);
...
const sortedFlights = useMemo(() => {
  ...
  const tableHeaders = useMemo(() => [
    ...
    { label: 'Destination', width: '300px', icon: MapPin        },
            ...
            <div className="flex-1 overflow-y-auto">
              {sortedFlights.length === 0 ? (
                ...
                sortedFlights.map((flight, index) => (
                  <FlightRow
                    // ── FIX (Chrome dugotrajan rad): key BEZ '-${index}'.
                    // Let (FlightNumber+ScheduledDepartureTime) je stabilan
                    // identitet — sa indexom u ključu, svaki put kad se
                    // board re-sortira (kašnjenje pomjeri let niže na
                    // listi, novi let se ubaci) React tretira ISTI let na
                    // novoj poziciji kao POTPUNO NOVI element -> uništava i
                    // ponovo pravi DOM čvor umjesto da ga samo premjesti.
                    // Board se re-sortira više puta u minuti, satima —
                    // konstantan nepotreban DOM churn (layout thrash, GC
                    // pritisak). combined/CombinedPageClientV2.tsx je već
                    // ispravno radio bez indexa; ovo poravnava departures
                    // na isti, bolji pattern.
