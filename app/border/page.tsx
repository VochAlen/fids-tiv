...
function computePill(flight: Flight): Pill {
  ...
  // ── 2. AUTO-STATUS ako je StatusEN prazan, "-", ili generički tekst ──
  // v4.5 FIX: raniji uslov (`!rawStatus || rawStatus === "-"`) je odustajao
  // čim je API vratio BILO KAKAV tekst statusa — a "On Time"/"Scheduled" su
  // upravo ono što API najčešće vraća prije nego let stvarno kasni/dolazi
  // ranije. Rezultat: diff-bazirano Delayed/Earlier računanje se skoro nikad
  // nije aktiviralo i tabla je uvijek prikazivala samo doslovni "On Time".
  let finalStatusText = rawStatus;
  const isGenericRawStatus = !rawStatus || rawStatus === "-" || /^(on time|na vrijeme|scheduled)$/i.test(rawStatus);
  if (isGenericRawStatus) {
...
// Filter letova — MORA biti definisan PRIJE useEffect-a koji ga koristi
const filter = useCallback((all: Flight[]): Flight[] => {
  ...
  return all.filter(f => {
    ...
    if (isCancelled) {
      const t = parseTime(f.ScheduledDepartureTime);
      if (!t) return false;
      return (now - t.getTime()) / 60_000 <= CANCELLED_SHOW_MINUTES;
    }
    if (isArrived) {
      const t = parseTime(f.ActualDepartureTime || f.EstimatedDepartureTime || f.ScheduledDepartureTime);
      if (!t) return false;
...
// Ably efekat — SADA ispod filter-a
const { data: liveFlightData } = useRealtimeFlightData('arrivals');

useEffect(() => {
  if (!liveFlightData?.arrivals) return;
  // eslint-disable-next-line react-hooks/set-state-in-effect
  setFlights(filter(liveFlightData.arrivals).slice(0, MAX_FLIGHTS_DISPLAY));
  // eslint-disable-next-line react-hooks/set-state-in-effect
  setLoading(false);
}, [liveFlightData, filter]);
          ...
          <div className="fids-table">
            ...
            <div className="fids-tbody">
              {sorted.length === 0 ? (
                ...
                // ── FIX (Chrome dugotrajan rad): key bez '-${i}' — vidi
                // komentar u app/departures/page.tsx.
