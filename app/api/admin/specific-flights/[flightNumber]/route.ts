...
// Tijelo PUT/PATCH zahtjeva za ažuriranje specifičnog leta — sva polja
// opciona, jer ruta ažurira SAMO ono što je stvarno poslato.
interface SpecificFlightUpdateBody {
  airlineIata?: string;
...
interface SpecificFlightUpdateData {
  updatedAt: Date;
  airlineIata?: string;
  alwaysBusinessClass?: boolean;
  winterOnly?: boolean;
...
function parseDaysOfWeek(daysOfWeek: unknown): number[] {
  try {
...
export async function PUT(
  ...
  try {
    ...
    const { flightNumber } = resolvedParams;
    const decodedFlightNumber = decodeFlightNumber(flightNumber);
    const data: SpecificFlightUpdateBody = await request.json();
    ...
    const updateData: SpecificFlightUpdateData = {
      updatedAt: new Date()
    };
