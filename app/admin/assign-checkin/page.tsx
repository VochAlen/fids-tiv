...
const StatsModal: React.FC<{
  ...
  // Spoji završene sesije + trenutno aktivne (koje još nisu u stats)
...
export default function AssignPanel() {
    ...
    const interval = setInterval(async () => {
      ...
      try {
        ...
        for (const a of checkinAssignmentsRef.current) {
          if (!sorted.find(f => f.FlightNumber === a.flightNumber)) {
            try {
              ...
              if (checkRes.ok) {
                ...
                if (current.flightNumber === a.flightNumber && current.status === 'open') {
                  await trackEnd('desk', a.resourceId);
          ...
          if (!sorted.find(f => f.FlightNumber === a.flightNumber)) {
            try {
              ...
              if (checkRes.ok) {
                const current = await checkRes.json();
                if (current.flightNumber === a.flightNumber && current.status === 'open') {
                  await trackEnd('gate', a.resourceId);
  ...
  const handleConfirmOverride = useCallback(async () => {
    ...
    if (!p) return;
    await trackEnd(p.resourceType, p.resourceId); // zatvori staru sesiju
  ...
  const handleRemoveCheckin = useCallback(async (deskNumber: string) => {
    try {
      await trackEnd('desk', deskNumber); // zapiši stats
      await fetch(`${API_PREFIX}/desk-status-override`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
  ...
  const handleRemoveGate = useCallback(async (gateNumber: string) => {
    try {
      await trackEnd('gate', gateNumber); // zapiši stats
      await fetch(`${API_PREFIX}/gate-status-override`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
...
{showStats && (
  <StatsModal
    ...
    isDark={isDark}
    currentCheckin={checkinAssignments}   // ← dodano
    currentGates={gateAssignments}        // ← dodano
  />
)}
