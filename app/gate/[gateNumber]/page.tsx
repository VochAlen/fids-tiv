...
function GateDisplay() {
  ...
  const gateResetTimerRef   = useRef<ReturnType<typeof setTimeout> | null>(null); // NOVI
  ...
  // ── AUTOMATSKO RESETOVANJE GATE-A IZ "open" U "auto" (null) ──
  const resetGateToAuto = useCallback(async () => {
    if (!gateNumber) return;
    try {
      await fetch(`/api/gate-status/${gateNumber}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ status: null })
      });
      manualGateStatusRef.current = null;
      await loadFlights(); // ponovo učitaj – sada bi departed trebao nestati
    } catch (err) {
      console.error('Auto gate reset failed:', err);
    }
  }, [gateNumber, loadFlights]);

  // Efekat koji postavlja timer za reset 2 minute prije polaska
  useEffect(() => {
    // Očisti stari timer
    if (gateResetTimerRef.current) {
      clearTimeout(gateResetTimerRef.current);
      gateResetTimerRef.current = null;
    }

    // Resetuj samo ako je gate ručno otvoren i postoji let
    if (manualGateStatusRef.current !== 'open') return;
    if (!display.flight) return;

    const flight = display.flight;
    const depTimeStr = flight.EstimatedDepartureTime || flight.ScheduledDepartureTime;
    if (!depTimeStr) return;

    const departureDate = parseDepartureTime(depTimeStr);
    if (!departureDate) return;

    // Resetuj 2 minute prije planiranog polaska
    const resetTime = departureDate.getTime() - 2 * 60 * 1000;
    const delay = resetTime - Date.now();

    if (delay <= 0) {
      // Već je prošlo vrijeme – resetuj odmah
      resetGateToAuto();
      return;
    }
  ...
  }, [display.flight, manualGateStatusRef.current, resetGateToAuto]);

  // Efekat za periodično osvježavanje (postoji već, ostaje nepromijenjen)
  useEffect(() => {
  ...
  // Timer za switch na sljedeći let tačno u STD (već postoji)
  useEffect(() => {
  ...
  // Polling za status gatea (već postoji)
  useEffect(() => {
    const poll = async () => {
      try {
      ...
      <div style={styles.main} className="fids-main">
        ...
        <div style={styles.leftCol} className="fids-left-col">
          ...
          <div style={styles.chargerWarning} className="fids-charger-warning">
            ...
            <span style={styles.chargerText}>Power banks: no overhead bins; keep under seat or in seat pocket. Do not use during flight.</span>
          </div>
          {/* ── BOARDING NOTICE ── */}
          <div style={styles.boardingNotice} className="fids-boarding-notice">
            <span style={styles.boardingIcon}>✈️</span>
            <span style={styles.boardingText}>Families with small children and elderly passengers may board first. If two stairs are used: rear section passengers (approx. rows B737-800 16+, A320 14+, A321 18+, E195 15+, may vary) use rear stairs; others use front. Thank you and have a pleasant flight 😊</span>
          </div>
        </div>
