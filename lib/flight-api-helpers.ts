...
dlog(`🕐 ${raw.BrojLeta}: input=${hours}:${minutes} | local=${scheduledDate.getHours()}:${scheduledDate.getMinutes()} | timestamp=${sortTime}`);
  ...
  return {
    ...
    StatusEN: (raw.StatusEN && raw.StatusEN !== '-' ? raw.StatusEN : null) || (raw.Status && raw.Status !== '-' ? raw.Status : null) || 'Scheduled',
    StatusMN: (raw.StatusMN && raw.StatusMN !== '-' ? raw.StatusMN : null) || 'Planiran',
...
export async function mapNgrokFlightToFlight(raw: NgrokFlightRaw): Promise<Flight> {
  ...
  return {
    ...
    StatusEN: (raw.comment && raw.comment !== '-' ? raw.comment : null) || 'Scheduled',
    StatusMN: 'Planiran',
    Terminal: '',
    GateNumber: raw.gate || '',
    GateNumbers: parseGateNumbers(raw.gate || ''),
...
export function filterTodayFlights(flights: Flight[]): Flight[] {
  // v4.5 FIX: ranije se koristio new Date().toDateString(), koji na Vercelu
  // (server radi u UTC-u) daje UTC datum, ne datum po lokalnom (Podgorica)
  // vremenu. Blizu ponoći/ranog jutra to može da "otkine" jedan dan ranije
  // ili kasnije nego što airport stvarno očekuje, baš u prozoru (04:00) kad
  // treba da se pređe na raspored NOVOG dana. getPodgoricaDateString koristi
  // se već svuda drugdje u projektu za "koji je danas dan" (vidi
  // flight-data-service.ts) — ovdje se poravnava sa istim principom.
  const today = getPodgoricaDateString();
  return flights.filter(f => {
    if (!f._sortTime) return true;
    return getPodgoricaDateString(new Date(f._sortTime)) === today;
  });
}