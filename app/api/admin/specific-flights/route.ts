...
import type { SpecificFlight } from '@/lib/db/schema';
...
function formatDateSimple(date: unknown): string {
  if (!date) return '';
...
function parseDateSimple(dateValue: unknown): Date | null {
...
export async function GET(request: NextRequest) {
  try {
    ...
    const formattedFlights = flights.map((flight: SpecificFlight) => {
